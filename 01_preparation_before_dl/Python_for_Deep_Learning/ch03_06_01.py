...
# 튜플 자료형
#튜플자료형은 리스트와 유사하지만 그 특징이 조금씩 다르다
#리스트는 대괄호[]를 이용하지만 튜플은 소괄호 ()를 사용한다
#리스트는 값을 변경할 수 있지만 튜플은 값을 변경할 수 없다.
#값이 변경되면 안 되는 경우 튜플을 사용하면 효과적이다
#괸행적으로 각 데이터의 성질이 다를 수 있을 때 일반적으로 튜플을 사용한다
#최단 경로 알고리즘에서 (비용, 노드번호)형태로 데이터를 묶어 사용하곤 한다
